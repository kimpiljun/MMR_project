...
1. **역할별 지표 가중치 부여**
- 포지션 별 주요 능력치로 볼 수 있는 지표 선정을 통한 MMR 계산 시 가중치 설정 로직
  - 각 포지션은 다른 스탯(예: 서포터의 시야 점수, 원거리 딜러의 골드 효율, 정글러의 오브젝트 컨트롤)을 강조합니다.
  - 가중치가 적용된 지표는 라인과 관계없이 공정한 영향력 평가를 보장합니다.
3. **랜덤 포레스트 기반 영향력 모델링**:
    특징 중요도를 도출하고 게임 영향력(GAME_IMPACT) 점수 모델을 구축하기 위해 랜덤 포레스트 분류기(RandomForestClassifier)를 활용.
    
4. **도메인 가중치 설정**: 특정 포지션에 따라 좀 더 중요하게 여겨지는 지표가 데이터를 다룬 해석으로 반영 X → 도메인 가중치 설정 !(ex . **서포터의 경우 ‘시야 점수’ 지표에 대한 가중치를 다른 포지션보다 2.5배 설정**)
5. **상대적 비교**: 상대방과 비교한 **상대 영향력 지표(impact_vs_str)** 생성
6. **MMR 조정 로직 설계**
