...
**‘리그 오브 레전드(LoL) 사용자 설정 게임 기반 MMR 지표 개발’** : 롤의 MMR 시스템은 비슷한 유저와 매칭되어 승패가 오르내리지만, 친구들과 즐기는 게임에서는 주 포지션을 맡기 어렵고 실력차도 존재한다. 따라서 단순 승패가 아닌 개인 활약도 기반 점수 시스템을 도입하여, 승리했지만 기여도가 낮다면 감점, 패배했지만 활약상이 높다면 가산점을 책정하는 시스템을 형성한다.
...
3. **도메인 가중치 할당**: 지표는 역할 상황 및 게임 상태(예: 초반 vs 후반)에 따라 동적인 가중치를 받습니다.
